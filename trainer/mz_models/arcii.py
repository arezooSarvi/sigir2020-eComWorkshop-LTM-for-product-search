...
model.params['kernel_2d_count'] = [32]
model.params['kernel_2d_size'] = [[3, 3]]
model.params['pool_2d_size'] = [[2, 2]]
model.params['optimizer'] = 'rmsprop'
model.params['dropout_rate'] = 0.18
...
mcp_save = keras.callbacks.ModelCheckpoint('best_one_arcii', save_best_only=True, monitor='val_loss', mode='min')
...
try:
    model.save('ARCII-model')
except:
    pass
write_predictions_in_test_score_file(test_set_predictions, item_query_ids)

# ---------------------
model.backend.load_weights("best_one_arcii", by_name=True)
test_set_predictions = model.predict(test_x)
