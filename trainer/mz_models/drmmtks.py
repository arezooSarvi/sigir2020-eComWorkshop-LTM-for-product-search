...
model.params['top_k'] = 49
model.params['mlp_num_layers'] = 6
model.params['mlp_num_units'] = 8
...
mcp_save = keras.callbacks.ModelCheckpoint('best_one_drmmtks', save_best_only=True, save_weights_only=True, monitor='val_loss', mode='min')
...
try:
    model.save('DRMMTKS-mode_default_params_early_stop')
except:
    pass
write_predictions_in_test_score_file(test_set_predictions, item_query_ids)
# ---------------------
model.backend.load_weights("best_one_drmmtks", by_name=True)
test_set_predictions = model.predict(test_x)
