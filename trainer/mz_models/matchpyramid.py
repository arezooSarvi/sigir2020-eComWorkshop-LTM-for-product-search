...
val_generator = mz.DataGenerator(validation_processed, callbacks=[dpool_callback])
...
mcp_save = keras.callbacks.ModelCheckpoint('best_one_matchpyramid', save_best_only=True, monitor='val_loss', mode='min')

data_generator = mz.DPoolPairDataGenerator(train_processed, fixed_length_left=20, fixed_length_right=20, num_dup=2, num_neg=2, batch_size=128)
...
model.save('MatchPyramid_model')
write_predictions_in_test_score_file(test_set_predictions, item_query_ids)

# ---------------------
model.backend.load_weights("best_one_matchpyramid", by_name=True)
test_set_predictions = model.predict(test_x)
